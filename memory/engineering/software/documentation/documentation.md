...
The in-code instance of *optimize for the reader / document to the interface* (see `software`): document the **interface and how to use it**, never the implementation — a reader should be able to tell whether this is the API they need, and how to call it, without opening the body.
...
- **Summary (`@brief`)** — one sentence naming precisely what it is or does: the caller's-eye view for a function, the kind-of-thing-and-its-defining-behavior for a type. This is the reader's mental model, so:
  - Define every domain term on first use, and never define a thing in terms of itself — coin it inline: "an append-only byte log for one stream of data, a.k.a. a Channel", not "a log for one channel".
  - Stay on the entity: describe what is declared, not its collaborators (point to related or consumer APIs with `@see` instead), and never let a member's summary echo the enclosing type's.
...
## Voice
- Write for a smart beginner: short sentences, plain words, no jargon, no hedging or filler. If a newcomer would stumble, simplify.
...
- State each fact once (*single source of truth* applied to prose — see `software`); never restate what the name, type, or signature already says (no `@param count an integer count`).
